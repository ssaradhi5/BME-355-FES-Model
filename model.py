...
class FESModel:
  def __init__(self, excitation_input, x_ext):
    ...
    self.external_state_vector = x_ext

  # EQN 4
  def roc_excitation(self, x):
    # x1 dot = rate of chance of dynamic level of muscle activation
    # x1 = Factivation
    x1 = x[0]
    x1_dot = (self.u - x1) * ((self.u / self.TAct) + ((1 - self.u) / self.TDeact))
  ...
  # EQN 6
  def rot_acceleration(self, state_vector, u):
    # Rotational Acceleration. Left is first term, right is last
    # TODO: Change paramaters to correct function names
    left = (1 / self.J) * (self.get_muscle_force(state_vector, u)) * self.d
    T_grav = self.tor_gravity(state_vector[1])
    T_acc = self.tor_ankle(self.external_state_vector, state_vector)
    T_ela = self.get_torque_elastic(state_vector)
    right = self.B * (self.external_state_vector[3] - state_vector[2])
  ...
  # EQN 8
  def tor_ankle(self, state_vector):
    # Torque of foot generated by ankle movement
    x1_ext = self.external_state_vector[0]
    x2_ext = self.external_state_vector[1]
    x2 = state_vector[1]
