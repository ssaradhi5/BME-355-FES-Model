...
class FESModel:
    ...
    def rot_acceleration(self, state_vector, x_ext_at_time):
        ...
        left = (1 / self.J) * \
            (self.get_muscle_force(state_vector, x_ext_at_time)) * self.d
    ...
    def get_muscle_force(self, x, x_ext_at_time):
        ...
        Fm = x[0] * self.FMax * Ffl * Ffv * \
            (x_ext_at_time[2] - x[1]) * (x_ext_at_time[3] - x[2])
...
def meanSquaredError(output_array, test_array):
    output_array[int(len(output_array)/2)                 :] = output_array[int(len(output_array)/2):]-25
