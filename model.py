...
class FESModel:
  ...
  def tor_gravity(self, state_vector):
    ...
    return (-self.mF * self.cF * math.cos(np.radians(x2)) * self.g)
  ...
  def get_muscle_force(self, x, x_ext_at_time):
    ...
    Fm = x[0] * self.FMax * Ffl * Ffv *(x_ext_at_time[2] - x[1])* (x_ext_at_time[3] - x[2])
...
def simulate(excitation_input, external_state_vectors, initial_state, simTime):
  ...
  sol = solve_ivp(model.get_derivative, simulationTime, initial_state, rtol=1e-5, atol=1e-8, max_step=1)
...
if __name__ == "__main__":
  ...
  excitation1 = np.genfromtxt('excitation-data-paper.csv_interpolated.csv', delimiter=',')
  ...
  plot_model(excitation1, x_ext, "Literature Excitation Signal")
  plot_model(excitation2, x_ext, "Trapezoid Excitation Signal")
  plot_model(excitation3, x_ext, "Constant Excitation Signal (u = 0.2)")
  plot_model(excitation4, x_ext, "Constant Excitation Signal (u = 1)")
  plt.show()
